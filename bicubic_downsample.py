...
class Downsampler:
  ...
  def resize_bicubic(self,im,img_size):
    factor = im.get_shape()[1].value//img_size
    if(factor*img_size != im.shape[1]):
      raise Exception("input size must be an integer multiple of output size")
    if(factor not in self.filter_dict):
      self.build_filter(factor)

    return self.apply_bicubic_downsample(im,self.filter_dict[factor],factor)
  ...
  def build_filter(self,factor):
    size = factor*4
    k = np.zeros((size))
    for i in range(size):
      x = (1/factor)*(i- np.floor(size/2) +0.5)
      k[i] = self.bicubic_kernel(x)
    k = k / np.sum(k)
    # make 2d
    k = np.outer(k, k.T)
    k = tf.constant(k, dtype=tf.float32, shape=(size, size, 1, 1))
    self.filter_dict[factor]=tf.concat([k, k, k], axis=2)

  def apply_bicubic_downsample(self,x, filter, factor):
